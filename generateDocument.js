...
const PAGE_W    = 11906; // A4 portrait width DXA
...
// ── Detail row (no-border 2-col table row) ──────────────────────────────────
...
// ── Main export ──────────────────────────────────────────────────────────────
async function generatePermitDocument(formData) {
  ...
  // ── Water quality table ──────────────────────────────────────────────────
  // Column widths: Reading | pH | NTU | Photo
  const WQ_READ  = 2000;
  const WQ_PH    = 1200;
  const WQ_NTU   = 1200;
  const WQ_PHOTO = CONTENT_W - WQ_READ - WQ_PH - WQ_NTU; // ~5346 DXA ≈ 3.71"

  // Photo display size (fit inside photo column with padding)
  const PHOTO_PX_W = 260;
  const PHOTO_PX_H = 195; // 4:3
  // Row height = photo height in DXA (1px ≈ 15 DXA at 96dpi)
  ...
  // ── Additional photos (one per page) ─────────────────────────────────────
...
module.exports = { generatePermitDocument };
